...
# Configurar logger
logger = logging.getLogger('pages.consultas')
logger.setLevel(logging.INFO)

st.set_page_config(page_title="Consultas", page_icon="🏥", layout="wide")

logger.info("Accediendo a módulo de Consultas")

if 'authenticated' not in st.session_state or not st.session_state.authenticated:
    logger.warning("Intento de acceso no autenticado a Consultas")
    st.warning("⚠️ Por favor, inicie sesión primero")
    st.stop()

# Control de acceso por rol
if st.session_state.user_type != "empleado":
    logger.warning(f"Intento de acceso a Consultas por usuario tipo: {st.session_state.user_type}")
...
logger.info(f"Usuario autenticado en Consultas: rol={user_role}, id={user_id}")
...
if user_role not in ['veterinario', 'enfermero', 'recepcionista']:
    logger.warning(f"Intento de acceso no autorizado a Consultas por rol={user_role}")
